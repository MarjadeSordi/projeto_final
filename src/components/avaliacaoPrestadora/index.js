...
import { ButtonModal, DivCapsule, InputForComent, InputForText, SelectedForNote } from './style';
...
const ClientAvaliacao = () => {
  ...
  function registerAvaliation(){
    ...
    const options = {
      ...
      fetch('http://whm.joao1866.c41.integrator.host:9206/avaliacao', options).
      then(result => {
        if (!result.ok) {
